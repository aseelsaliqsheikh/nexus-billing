...
def generate_pdf(doc_type, doc_num, client_name, client_phone, client_gstin, client_state, doc_date, items, subtotal, tax_amt, grand_total, bank_details=None, is_duplicate=False, theme="Editorial Dark & Asymmetric", is_non_tax=False, page_size_name="A4"):
    ...
    # Build PDF layout elements matching the requested editorial dark styling
    comp_title_style = ParagraphStyle('CompTitle', parent=styles['Heading1'], fontSize=14, leading=17, textColor=colors.white, fontName="Helvetica-Bold")
...
def render_html_preview(doc_type, doc_num, client_name, client_phone, client_gstin, client_state, doc_date, items, subtotal, tax_amt, grand_total, bank_details=None, is_duplicate=False, theme="Editorial Dark & Asymmetric", is_non_tax=False, page_size_name="A4"):
    ...
    for idx, item in enumerate(items, start=1):
        ...
        if is_non_tax:
            items_html += f"""
                <tr>
                    <td style="padding: 12px 0; border-bottom: 1px solid rgba(51, 65, 85, 0.4); font-family: sans-serif; color: #cbd5e1;">{item['desc']}</td>
                    <td style="padding: 12px 0; border-bottom: 1px solid rgba(51, 65, 85, 0.4); text-align: right;">{item['qty']}</td>
                    <td style="padding: 12px 0; border-bottom: 1px solid rgba(51, 65, 85, 0.4); text-align: right; font-weight: 600;">Rs. {line_sub:,.2f}</td>
                </tr>
            """
        else:
            line_tax = line_sub * (item['tax_rate'] / 100)
            ...
            items_html += f"""
                <tr>
                    <td style="padding: 12px 0; border-bottom: 1px solid rgba(51, 65, 85, 0.4); font-family: sans-serif; color: #cbd5e1;">{item['desc']}</td>
                    <td style="padding: 12px 0; border-bottom: 1px solid rgba(51, 65, 85, 0.4); text-align: right;">{item['qty']}</td>
                    <td style="padding: 12px 0; border-bottom: 1px solid rgba(51, 65, 85, 0.4); text-align: right;">Rs. {item['rate']:,.2f}</td>
                    <td style="padding: 12px 0; border-bottom: 1px solid rgba(51, 65, 85, 0.4); text-align: right; font-weight: 600;">Rs. {line_total:,.2f}</td>
                </tr>
            """

    header_th = """
        <th style="text-align: left; font-size: 0.8rem; text-transform: uppercase; color: #94a3b8; padding-bottom: 10px; border-bottom: 1px solid #334155;">Item Description</th>
        <th style="text-align: right; font-size: 0.8rem; text-transform: uppercase; color: #94a3b8; padding-bottom: 10px; border-bottom: 1px solid #334155;">Qty</th>
        <th style="text-align: right; font-size: 0.8rem; text-transform: uppercase; color: #94a3b8; padding-bottom: 10px; border-bottom: 1px solid #334155;">Amount</th>
    """ if is_non_tax else """
        <th style="text-align: left; font-size: 0.8rem; text-transform: uppercase; color: #94a3b8; padding-bottom: 10px; border-bottom: 1px solid #334155;">Item Description</th>
        <th style="text-align: right; font-size: 0.8rem; text-transform: uppercase; color: #94a3b8; padding-bottom: 10px; border-bottom: 1px solid #334155;">Qty</th>
        <th style="text-align: right; font-size: 0.8rem; text-transform: uppercase; color: #94a3b8; padding-bottom: 10px; border-bottom: 1px solid #334155;">Rate</th>
        <th style="text-align: right; font-size: 0.8rem; text-transform: uppercase; color: #94a3b8; padding-bottom: 10px; border-bottom: 1px solid #334155;">Total</th>
    """

    html_content = f"""
    <div style="max-width: 850px; margin: auto; background: #1e293b; color: #f8fafc; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; border-radius: 12px; overflow: hidden; display: grid; grid-template-columns: 280px 1fr; border-left: 6px solid #3b82f6; box-shadow: 0 20px 25px -5px rgba(0,0,0,0.5);">
        <div style="background: #111827; padding: 35px 25px; border-right: 1px solid #334155; display: flex; flex-direction: column; justify-content: space-between;">
            <div>
                <h1 style="font-size: 1.35rem; font-weight: 800; margin: 0 0 8px 0; color: #ffffff; text-transform: uppercase;">{comp_name_str}</h1>
                <p style="font-size: 0.8rem; color: #94a3b8; line-height: 1.4; margin: 0 0 25px 0;">{comp_sub_str}</p>
                
                <div style="font-size: 0.75rem; text-transform: uppercase; color: #94a3b8; margin-bottom: 4px;">Document Number</div>
                <div style="font-size: 0.9rem; font-weight: 600; margin-bottom: 16px;">{doc_num}</div>

                <div style="font-size: 0.75rem; text-transform: uppercase; color: #94a3b8; margin-bottom: 4px;">Issue Date</div>
                <div style="font-size: 0.9rem; font-weight: 600; margin-bottom: 16px;">{doc_date}</div>

                <div style="font-size: 0.75rem; text-transform: uppercase; color: #94a3b8; margin-bottom: 4px;">Supplier State</div>
                <div style="font-size: 0.9rem; font-weight: 600;">{comp_state_str}</div>
            </div>
            <div style="font-size: 0.75rem; color: #64748b; margin-top: 30px;">
                Secure Digital Billing Suite<br/>Bangalore, Karnataka
            </div>
        </div>

        <div style="padding: 35px; background-image: radial-gradient(circle at 90% 10%, rgba(59, 130, 246, 0.05) 0%, transparent 40%); display: flex; flex-direction: column; justify-content: space-between; position: relative;">
            <div style="position: absolute; right: 20px; bottom: 40px; font-size: 5rem; font-weight: 900; color: rgba(255, 255, 255, 0.02); pointer-events: none; text-transform: uppercase;">NEXUS</div>
            
            <div>
                <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 30px; border-bottom: 1px solid #334155; padding-bottom: 15px;">
                    <div>
                        {dup_banner}
                        <h2 style="font-size: 1.8rem; font-weight: 700; margin: 0; letter-spacing: -0.03em;">{doc_type.upper()}</h2>
                    </div>
                    <div>
                        <span style="display: inline-flex; align-items: center; padding: 6px 14px; border-radius: 9999px; background-color: rgba(59, 130, 246, 0.15); color: #60a5fa; font-size: 0.75rem; font-weight: 600;">Verified &amp; Processed</span>
                    </div>
                </div>

                <div style="margin-bottom: 25px;">
                    <div style="font-size: 0.75rem; text-transform: uppercase; color: #94a3b8; margin-bottom: 4px;">Billed To</div>
                    <div style="font-size: 1rem; font-weight: 600;">{client_name}</div>
                    <div style="font-size: 0.85rem; color: #94a3b8; margin-top: 2px;">State: {client_state if client_state else 'Karnataka'}</div>
                </div>
            ...
            <div style="display: flex; justify-content: flex-end; margin-top: 20px;">
                <div style="width: 250px; border-left: 3px solid #3b82f6; padding-left: 15px;">
                    ...
                    {"<div style='display: flex; justify-content: space-between; margin-bottom: 8px; font-size: 0.85rem; color: #94a3b8;'><span>Tax</span><span>Rs. " + f"{tax_amt:,.2f}" + "</span></div>" if not is_non_tax else ""}
                    <div style="display: flex; justify-content: space-between; margin-top: 10px; padding-top: 10px; border-top: 1px solid #334155; font-size: 1.1rem; font-weight: 700;">
                        <span>Total Due</span>
...
selected_theme = st.sidebar.selectbox(
    ...
    ["Editorial Dark & Asymmetric", "Modern Minimalist (Clean Slate)", "Executive Dark (Bold & Corporate)", "Creative Vibrant (Blue & Slate)"],
    index=0
)
